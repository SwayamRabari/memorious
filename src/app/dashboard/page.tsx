...
const Dashboard = () => {
  ...
  const fetchNotes = async () => {
    if (demo) {
      const demoNotes = [
        {
          id: '1',
          title: 'Welcome to Memorious!',
          content: `
  <p class="welcome-paragraph">
    You are currently in <strong>demo mode</strong>
  </p>
  <p>This mode is for demonstration purposes to showcase the app's features.</p>
  <p>Here are some things you can do:</p>
  <ul class>
    <li><strong>Create</strong> new notes to organize your thoughts.</li>
    <li><strong>Edit</strong> existing notes to refine your ideas.</li>
    <li><strong>Delete</strong> notes when you no longer need them.</li>
    <li>
          Utilize <strong>Gemini AI</strong> to generate notes effortlessly from your prompts.</li>
  </ul>
  <p>
      Have any feedback? Email us at
      <a href="mailto:memorious.so@gmail.com" className="text-blue-500 underline">
        memorious.so@gmail.com
      </a>
    </p>`,
        },
      ];
      setNotesArray(demoNotes);
    ...
    try {
      ...
      if (demo) {
        if (selectedNote.id && !selectedNote.id.startsWith('temp-')) {
          ...
          const newNote = { ...selectedNote, id: Date.now().toString() };
          setSelectedNote(newNote);
          setNotesArray((prev) => [
            newNote,
            ...prev.filter((x) => !x.id.startsWith('temp-')),
          ]);
        }
        setHasUnsavedChanges(false);
        return;
      ...
      if (selectedNote.id && !selectedNote.id.startsWith('temp-')) {
        toast.loading('Saving...', { id: 'save-note' });
        ...
        toast.loading('Saving...', { id: 'create-note' });
  ...
  const handleDeleteNote = useCallback(async () => {
    ...
    toast.loading('Deleting...', { id: 'delete-note' });
    try {
      const response = await fetch(`/api/notes/${selectedNote.id}`, {
        method: 'DELETE',
      });
